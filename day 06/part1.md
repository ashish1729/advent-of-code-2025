...
vector<string> split(string& s, char sep = ' ') {
    ...
    for (char c : s) {
        if (c == sep) { if (curr != " " and curr != "")ans.push_back(curr); curr = ""; }
        else { curr += c; }
    }
    if (curr != " " and curr != "") ans.push_back(curr);
    return ans;
}
// Input stuff
...
void solve(int TC) {
    ...
    while (getline(cin, line)) {
        t = split(line);
        if (not isdigit(t[0][0])) break;

        vl temp;
        iter(t) temp.pb(stoll(x));
        ar.pb(temp);
    }

    int n = len(ar), m = len(ar[0]);
    ll ans = 0;
    til(i, m) {
        if (t[i] == "+") {
            ll res = 0;
