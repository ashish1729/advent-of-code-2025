...
int clean(vs& ar) {
    ...
    glen(ar, n); glen(ar[0], m);

    char paper = '@', empty = '.';

    auto isPaper = [&](int x, int y) {
        if (not inGrid(x, y, n, m)) return false;
        return ar[x][y] == paper;
        };

    auto  cnt = [&](int a, int b) {
        int ans = 0;
        for (auto dir : eightDirections) ans += isPaper(a + dir[0], b + dir[1]);
        return ans;
        };
    ...
    vii rem;
    til(i, n) til(j, n) {
        if (ar[i][j] == paper and cnt(i, j) < 4) {
            rem.pb({ i,j });
        }
    }

    iter(rem) {
        ar[x.first][x.second] = empty;
    }
