...
- kind of direct application of https://leetcode.com/problems/remove-k-digits/description/
...
string getAns(string& s, int k) {
    int n = len(s), canRemove = n - k;
    string ans = "";

    // we are kind of greedily replacing earlier smaller numbers with bigger number
    // BUT ONLY IF we have `canRemove > 0` which means we can remove more chars
    iter(s) {
        while (true) {
            if (ans.empty() or canRemove == 0 or ans.back() >= x) break;
            ans.pop_back(); canRemove--;
        }
        ans += x;
    }
...
void solve(int TC) {
    ll ans = 0;
    string s;
    while (cin >> s)  ans += stoll(getAns(s, 12));
    dbg(ans);
}
