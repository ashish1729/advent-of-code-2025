- doesn't work on small input
- large input is a joke/trick/prank that doesn't need any tight fitting
...
void solve(int TC) {
    ...
    while (getline(cin, line)) {
        ...
        vs two = split(line, ' ');
        int a = stoi(two[0].substr(0, 2));
        int b = stoi(two[0].substr(3, 2));
        // dbg(a, b);
        int tot = 0;
        FOR(i, 1, 7) tot += stoi(two[i]);
        int grid = (a / 3) * (b / 3);
        if (grid >= tot)++ans;
    }
